...
void MapReduce (sequential_execution s, InputIt first, InputIt last, OutputIt firstOut, MapFunc const & map, ReduceOperator op, MoreIn ... inputs) {
    while( first != last ) {
...
inline T MapReduce ( sequential_execution, InputIt first, InputIt last, MapFunc const &  map, T init, ReduceOperator op){
    T out = init;
...
template <typename InputIt, typename MapFunc, class T, typename ReduceOperator>
inline T MapReduce ( sequential_execution, InputIt first, InputIt last, MapFunc const &  map, ReduceOperator op){
    T out;  
