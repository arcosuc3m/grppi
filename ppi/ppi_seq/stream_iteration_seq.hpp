...
template<typename GenFunc, typename TaskFunc, typename Predicate, typename OutFunc>
inline void StreamIteration(sequential_execution, GenFunc const & in, FarmObj<sequential_execution, TaskFunc> const & f, Predicate const & condition, OutFunc const & out){
   while(1){
       auto k = in();       
       if(!k) break;
       auto val = k.value();
