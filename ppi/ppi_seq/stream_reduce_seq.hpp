...
#include "reduce_seq.hpp"
#include <vector>
using namespace std;
namespace grppi{
template <typename GenFunc, typename TaskFunc, typename ReduceFunc, typename OutputType>
inline void StreamReduce(sequential_execution s, GenFunc const &in, TaskFunc const & taskf, ReduceFunc const &red, OutputType &reduce_value ) {
...
inline void StreamReduce(sequential_execution s, GenFunc const &in, TaskFunc const & taskf, ReduceFunc const &red) {
    while( 1 ) {
        auto k = in();
        if( !k )
            break;
...
inline void StreamReduce(sequential_execution s, GenFunc const &in, int windowsize, int offset, ReduceOperator const & op, SinkFunc const &sink)
{
