...
#include "reduce_tbb.hpp"
#include <tbb/tbb.h>

using namespace std;
namespace grppi{
template <typename GenFunc, typename TaskFunc, typename ReduceFunc, typename OutputType>
inline void StreamReduce(parallel_execution_tbb p, GenFunc const &in, TaskFunc const &taskf, ReduceFunc const &red, OutputType &reduce_value ){
...
inline void StreamReduce(parallel_execution_tbb s, GenFunc const &in, int windowsize, int offset, ReduceOperator const & op, SinkFunc const &sink)
{
...
ReduceObj<parallel_execution_tbb,TaskFunc, RedFunc> StreamReduce(parallel_execution_thr p, TaskFunc && taskf, RedFunc && red){
