...
inline void InternalMap(parallel_execution_omp p, InputIt first, InputIt last, OutputIt firstOut, TaskFunc const & taskf, int i, int elemperthr, MoreIn ... inputs){
...
inline void Map(parallel_execution_omp p, InputIt first, InputIt last, OutputIt firstOut, TaskFunc const & taskf, MoreIn ... inputs){
   ...
   #pragma omp single nowait
   {
     for(int i=1;i<p.num_threads;i++){
       ...
       #pragma omp task firstprivate(i)
       {
           InternalMap(p, first, last, firstOut, taskf, i, elemperthr, inputs ...);
       }
      //End task
     }
     //Map main thread
     InternalMap(p, first,last, firstOut, taskf, 0, elemperthr, inputs ...);
