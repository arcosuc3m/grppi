...
inline void InternalDivideAndConquer(parallel_execution_omp p, Input & problem, Output & output,
    ...
    if(subproblems.size()>1){
         ...
         for(i; i != subproblems.end()&& num_threads.load()>0; i++, division++){
            //THREAD
             ...
             {
                  InternalDivideAndConquer(p, *i, partials[division], divide, task, merge, num_threads);
              //END TRHEAD
             }
             num_threads --;
          ...
          for(i; i != subproblems.end(); i++){
              DivideAndConquer(sequential_execution {},*i,partials[division], divide, task, merge);
          }
          
          //Main thread works on the first subproblem.
          InternalDivideAndConquer(p, *subproblems.begin(), partials[0], divide, task, merge, num_threads);
        ...
        DivideAndConquer(sequential_execution {}, problem, output, divide, task, merge);
     }
...
template <typename Input, typename Output, typename DivFunc, typename TaskFunc, typename MergeFunc>
inline void DivideAndConquer(parallel_execution_omp p, Input & problem, Output & output,
    ...
    if(num_threads.load()>0){
      ...
      if(subproblems.size()>1){
         ...
         if(num_threads.load()>0){
            ...
            DivideAndConquer(sequential_execution {}, *subproblems.begin(), partials[0], divide, task, merge);
        }
          #pragma omp taskwait
          }
          }
