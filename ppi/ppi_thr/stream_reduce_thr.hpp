...
#include <thread>
#include "reduce_thr.hpp"
using namespace std;
namespace grppi{
template <typename GenFunc, typename TaskFunc, typename ReduceFunc, typename OutputType>
inline void StreamReduce(parallel_execution_thr &p, GenFunc const &in, TaskFunc const &taskf, ReduceFunc const &red, OutputType &reduce_value ){
...
inline void StreamReduce(parallel_execution_thr &p, GenFunc const &in, int windowsize, int offset, ReduceOperator const & op, SinkFunc const &sink)
{
