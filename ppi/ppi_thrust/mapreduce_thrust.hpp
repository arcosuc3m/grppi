...
inline void MapReduce (parallel_execution_thrust_internal<Policy>  p, InputIt first, InputIt last, OutputIt firstOut, MapFunc const & map, RedFunc const & reduce, MoreIn ... inputs) {
...
inline void MapReduce (parallel_execution_thrust_internal<Policy>  p, InputIt first, InputIt last,  MapFunc const & map, RedFunc const & reduce, Output & out) {
       thrust::device_vector< typename InputIt::value_type > d_vec_in(first, last);
