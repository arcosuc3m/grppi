 ...
 */
  ...
  public:
    ...
    */
    parallel_execution_tbb() noexcept = default;
    ...
    */
    explicit parallel_execution_tbb(int concurrency_degree, bool order = true)
    noexcept
        :
    ...
    */
    [[nodiscard]] bool is_ordered() const noexcept { return ordering_; }
    ...
    */
    [[nodiscard]] int tokens() const noexcept { return num_tokens_; }
    ...
    */
    ...
    that uses multiple-producer/multiple-consumer_queues as communication
     channels.
...
*/
  template<>
  constexpr bool
  supports_divide_conquer<parallel_execution_tbb>() { return true; }
...
*/
      ...
      std::size_t sequence_size, Transformer transform_op) const
  {
    tbb::parallel_for(
        ...
        [&](std::size_t index) {
          first_out[index] = apply_iterators_indexed(transform_op, firsts,
              index);
        }
    );
      ...
      Combiner && combine_op) const
  {
    ...
    return tbb::parallel_reduce(
        tbb::blocked_range<InputIterator>(first,
            std::next(first, sequence_size)),
        identity,
...
// PRIVATE MEMBERS
      ...
      std::atomic<int> & num_threads) const
  {
    ...
    tbb::task_group g;
    auto i = sub_problems.begin() + 1;
    while (i != sub_problems.end() && num_threads.load() > 0) {
    ...
    //Main thread works on the first sub-problem.
    while (i != sub_problems.end()) {
      partials[division] = seq.divide_conquer(std::forward<Input>(*i++),
          std::forward<Divider>(divide_op), std::forward<Solver>(solve_op),
          std::forward<Combiner>(combine_op));
    }

    auto out = divide_conquer(std::forward<Input>(*sub_problems.begin()),
        std::forward<Divider>(divide_op), std::forward<Solver>(solve_op),
        std::forward<Combiner>(combine_op), num_threads);
      ...
      std::atomic<int> & num_threads) const
  {
    ...
    //Main thread works on the first sub_problem.
    while (i != sub_problems.end()) {
      partials[division] = seq.divide_conquer(std::forward<Input>(*i++),
          std::forward<Divider>(divide_op),
          std::forward<Predicate>(predicate_op),
