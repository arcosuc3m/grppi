...
*/
  ...
  public:
    ...
    */
    ...
    constexpr void
    stencil(std::tuple<InputIterators...> firsts, OutputIterator first_out,
        std::size_t sequence_size,
        ...
        mpmc_queue<OutputType> & output_queue) const
    {
      ...
      for (;;) {
        ...
        do_pipeline(*item.first, std::forward<Transformer>(transform_op),
            [&](auto output_item) {
              output_queue.push(
                  make_pair(optional_output_type{output_item}, item.second));
            }
        );
      }
...
*/
  template<>
  constexpr bool
  supports_divide_conquer<sequential_execution>() { return true; }
      ...
      Combiner && combine_op) const
  {
    ...
    for (auto && sp : subproblems) {
      solutions.push_back(divide_conquer(sp,
          ...
          std::forward<Predicate>(predicate_op), std::forward<Solver>(solve_op),
          std::forward<Combiner>(combine_op)));
    }
    return reduce(std::next(solutions.begin()), solutions.size() - 1,
        solutions[0],
        std::forward<Combiner>(combine_op));
  }
