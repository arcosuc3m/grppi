...
#ifndef GRPPI_CUDA_PARALLEL_EXECUTION_CUDA_H
#define GRPPI_CUDA_PARALLEL_EXECUTION_CUDA_H
...
This policy uses CUDA Thrust as implementation back-end.
*/
class parallel_execution_cuda {

public:
  /** 
  \brief Default construct an CUDA Thrust parallel execution policy.

  Creates an Thrust parallel execution object.

  */
  parallel_execution_cuda() noexcept :
      parallel_execution_cuda{impl_concurrency_degree()}
  {}
  ...
  */
  parallel_execution_cuda(int concurrency_degree, bool order = true) noexcept :
...
private:
  ...
  */
  static int impl_concurrency_degree() {
    int result  = 1; // TODO: In the future we can support multiple devices
    return result;
  }
...
/**
\brief Metafunction that determines if type E is parallel_execution_cuda
\tparam Execution policy type.
*/
template <typename E>
constexpr bool is_parallel_execution_cuda() {
  return std::is_same<E, parallel_execution_cuda>::value;
}

/**
\brief Determines if an execution policy is supported in the current compilation.
\note Specialization for parallel_execution_cuda when GRPPI_CUDA is enabled.
*/
template <>
constexpr bool is_supported<parallel_execution_cuda>() { return true; }
...
*/
template <>
constexpr bool supports_map<parallel_execution_cuda>() { return true; }
...
*/
template <>
constexpr bool supports_reduce<parallel_execution_cuda>() { return true; }
...
*/
template <>
constexpr bool supports_map_reduce<parallel_execution_cuda>() { return true; }
...
*/
template <>
constexpr bool supports_stencil<parallel_execution_cuda>() { return true; }
...
*/
template <>
constexpr bool supports_divide_conquer<parallel_execution_cuda>() { return true; }
...
*/
template <>
constexpr bool supports_pipeline<parallel_execution_cuda>() { return true; }

template <typename ... InputIterators, typename OutputIterator, 
          typename Transformer>
void parallel_execution_cuda::map(
    std::tuple<InputIterators...> firsts,
    OutputIterator first_out, 
    std::size_t sequence_size, Transformer transform_op) const
{

  constexpr int first_size = std::tuple_size<std::tuple<InputIterators...>>::value;
  //static_assert(first_size > 3, "Not supported more than 2 input vectors");
  thrust::device_vector<typename OutputIterator::value_type> in(std::get<0>(firsts), std::next(std::get<0>(firsts), sequence_size));
  thrust::device_vector<typename OutputIterator::value_type> out{sequence_size};
  thrust::transform(thrust::device, in.begin(), in.end(), out.begin(), transform_op);

  thrust::copy_n(out.begin(), sequence_size, first_out);
}

template <typename InputIterator, typename Identity, typename Combiner>
auto parallel_execution_cuda::reduce(
    ...
    Combiner && combine_op) const
{
  ...
  thrust::device_vector<typename InputIterator::value_type> in(first, next(first), sequence_size);
 
  return thrust::reduce(in.begin(), in.end(), std::forward<Identity>(identity), std::forward<Combiner>(combine_op));
}

template <typename ... InputIterators, typename Identity, 
          typename Transformer, typename Combiner>
auto parallel_execution_cuda::map_reduce(
    std::tuple<InputIterators...> firsts,
    std::size_t sequence_size,
    Identity && identity,
    Transformer && transform_op, Combiner && combine_op) const
...
template <typename ... InputIterators, typename OutputIterator,
          typename StencilTransformer, typename Neighbourhood>
void parallel_execution_cuda::stencil(
...
template <typename Input, typename Divider, typename Solver, typename Combiner>
auto parallel_execution_cuda::divide_conquer(
    Input && input, 
    Divider && divide_op, 
    Solver && solve_op, 
    Combiner && combine_op) const
...
template <typename Generator, typename ... Transformers>
void parallel_execution_cuda::pipeline(
...
template <typename Queue, typename Consumer,
          requires_no_pattern<Consumer> =0>
void parallel_execution_cuda::do_pipeline(Queue & input_queue, Consumer && consume_op) const
{
  using namespace std;
...
template <typename Queue, typename Transformer, typename ... OtherTransformers,
          requires_no_pattern<Transformer> =0>
void parallel_execution_cuda::do_pipeline(
    Queue & input_queue, 
    ...
    OtherTransformers && ... other_transform_ops) const
{
  ...
  auto reduce_task = [&,this]() {
    ...
    int order = 0;
    while (item.first) {
      std::cerr << "item: " << *item.first << " , " << item.second << "\n";
...
template <typename Queue, typename ... Transformers,
          std::size_t ... I>
void parallel_execution_cuda::do_pipeline_nested(
    Queue & input_queue, 
    std::tuple<Transformers...> && transform_ops,
    std::index_sequence<I...>) const
{
...
#else // GRPPi_CUDA undefined

namespace grppi {


/// Parallel execution policy.
/// Empty type if  GRPPI_CUDA disabled.
struct parallel_execution_cuda {};

/**
\brief Metafunction that determines if type E is parallel_execution_cuda
This metafunction evaluates to false if GRPPI_CUDA is disabled.
\tparam Execution policy type.
*/
template <typename E>
constexpr bool is_parallel_execution_cuda() {
