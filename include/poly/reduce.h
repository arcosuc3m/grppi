...
template <typename InputIt, typename OutputIt, typename ReduceOperator>
void reduce_multi_impl(polymorphic_execution & e, InputIt first, InputIt last, 
         OutputIt first_out, ReduceOperator op) 
{
}
...
template <typename E, typename ... O,
          typename InputIt, typename OutputIt, typename ReduceOperator,
          internal::requires_execution_not_supported<E> = 0>
void reduce_multi_impl(polymorphic_execution & e, InputIt first, InputIt last, 
         OutputIt first_out, ReduceOperator op) 
{
  map_multi_impl<O...>(e, first, last, first_out, op);
}
...
void reduce_multi_impl(polymorphic_execution & e, InputIt first, InputIt last, 
         ReduceOperator op) 
{
  map_multi_impl<O...>(e, first, last, op);
}
...
{
  map_multi_impl<O...>(e, first, last, first_out, std::forward<RedFunc>(reduce));
}
         ...
         OutputIt first_out, ReduceOperator op) 
{
  if (typeid(E) == e.type()) {
    reduce(*e.execution_ptr<E>(), 
        first, last, first_out, op);
  }
  else {
    reduce_multi_impl<O...>(e, first, last, first_out, op);
  }
}
...
template <typename InputIt, typename OutputIt, typename ReduceOperator>
void reduce(polymorphic_execution & e, InputIt first, InputIt last, 
         OutputIt first_out, ReduceOperator op) 
{
  map_multi_impl<
