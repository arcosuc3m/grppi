...
template < typename InputIt, typename OutputIt, typename MapFunc, typename RedFunc, typename Policy, typename ... MoreIn >
 void map_reduce (parallel_execution_thrust_internal<Policy>  p, InputIt first, InputIt last, OutputIt firstOut, MapFunc && map, RedFunc && reduce, MoreIn ... inputs) {
...
template < typename InputIt, typename Output, typename MapFunc, typename RedFunc, typename Policy >
 void map_reduce (parallel_execution_thrust_internal<Policy>  p, InputIt first, InputIt last,  MapFunc && map, RedFunc && reduce, Output & out) {
       thrust::device_vector< typename InputIt::value_type > d_vec_in(first, last);
