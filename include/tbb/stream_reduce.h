...
template <typename Generator, typename Combiner, typename Consumer, typename Identity>
 void stream_reduce(parallel_execution_tbb &s, Generator &&gen, int windowsize, int offset, Combiner && comb, Consumer &&cons, Identity init)
{
     ...
     while(1){
        ...
        if(buffer.size()>0){
           ...
           auto reduceVal = reduce(s, buffer.begin(), buffer.end(), init, std::forward<Combiner>(comb));
