    ...
    std::size_t sequence_size, Transformer transform_op) const
{
  ...
  for(unsigned int i=0; i< sequence_size / chunk_size; ++i){
    ...
    // --- Demo ---
    std::stringstream ss;
    ss<< "Create task: "<< i << " data location "<< pos << std::endl;
    print(ss.str());
     
    //Store data in data manager
    data_manager[pos] = make_tuple(aux_in, chunk_size, aux_out);
    task_type t{function_id, i}; //scheduler.get_task_id()};
...
/*  } else {
    ...
    long current = 0;
    (void) scheduler.register_sequential_task([input_queue, &consume_op, &current, this](task_type t){
       //TODO: NOT A GOOD CHOICE -> This will result on multiple executions of useless work 
       auto item=input_queue->pop();
       if( current == item.second)
       {
         ...
         scheduler.launch_task(t.get_id());
       }
       scheduler.notify_seq_end(t);
    });
  }
