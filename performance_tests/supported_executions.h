...
grppi::dynamic_execution execution_mode(const std::string & opt, int conc_degr=0, bool order=true) {
  ...
  if ("omp" == opt) { 
  	if(conc_degr != 0)
  		return parallel_execution_omp(conc_degr+2, order);
  	else {
  ...
  if ("task" == opt) { 
  	if(conc_degr != 0){
  		return parallel_execution_task<fifo_scheduler<simple_task>>(fifo_scheduler<simple_task>{},24, order);
    }
  	else {
